...
def parse_alipay_csv(filepath: str, db: Session) -> dict:
    ...
    log_action(
        db,
        ACTION_IMPORT_CSV,
        f"导入支付宝账单：共 {len(df)} 条，成功 {success_count} 条，跳过重复 {skip_count} 条，过滤 {filtered_count} 条",
        {"source": "alipay", "total": len(df), "success": success_count, "skipped": skip_count, "filtered": filtered_count, "transaction_ids": inserted_ids},
    )
